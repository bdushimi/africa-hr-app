...
public class S3ServiceImpl implements S3ServiceInterface {
        ...
        public PresignedUrlResponse generatePresignedUploadUrl(PresignedUrlRequest request) {
                try {
                        // Call the existing method with parameters from the request
                        Map<String, String> presignedData = generatePresignedUrl(
                                        request.getFileName(),
                                        request.getContentType(),
                                        request.getFolder(),
                                        request.getExpiresIn(),
                                        null); // No content length in the request

                        // Set expiration timestamp
                        LocalDateTime expiresAt = LocalDateTime.now().plusSeconds(
                                        request.getExpiresIn() != null ? request.getExpiresIn() : 3600);

                        // Build response from the returned data
                        return PresignedUrlResponse.builder()
                                        .presignedUrl(presignedData.get("url"))
                                        .objectKey(presignedData.get("fileKey"))
        ...
        public DownloadUrlResponse generatePresignedDownloadUrl(String objectKey, DownloadUrlRequest request) {
                try {
                        // Call the existing method with parameters from the request
                        Map<String, Object> downloadData = generatePresignedDownloadUrl(
                                        objectKey,
                                        request.getExpiresIn(),
                                        request.getFileName(),
                                        request.getInline());
                        ...
                        ZonedDateTime zonedExpiresAt = (ZonedDateTime) downloadData.get("expiresAt");
                        LocalDateTime expiresAt = zonedExpiresAt != null
                                        ? zonedExpiresAt.toLocalDateTime()
                                        : LocalDateTime.now().plusSeconds(
                                                        request.getExpiresIn() != null ? request.getExpiresIn() : 3600);

                        // Build response from the returned data
                        return DownloadUrlResponse.builder()
                                        .downloadUrl((String) downloadData.get("downloadUrl"))
                                        .fileName((String) downloadData.get("fileName"))
                                        .contentType((String) downloadData.get("contentType"))
                                        .expiresAt(expiresAt)
                                        .build();
                } catch (Exception e) {
        ...
        public boolean deleteObject(String objectKey) {
                try {
                        ...
                        log.info("Successfully deleted object with key: {}", objectKey);
                        return true;
                } catch (Exception e) {
                        log.error("Error deleting object from S3 with key: {}", objectKey, e);
                        return false;
                }
        }
}